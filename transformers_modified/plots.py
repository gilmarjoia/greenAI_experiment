"""
plots.py — Generate training artifacts for transformers_modified.
Identical to transformers_baseline/plots.py — only the chart title is updated.
...
def plot_results(results: list[dict], save_dir: Path) -> None:
    """4-panel plot matching YOLO's results.png layout."""
    ...
    fig.suptitle("ViT Modified — Training Results", fontsize=13, fontweight="bold")
...
def _save_batch_image(
    ...
    for i in range(n):
        ...
        if preds is not None:
            label_text.append(f"{true_name}→{class_names[preds[i]]}")
        else:
...
def plot_batch_images(
    ...
    for i in range(n_batches):
        try:
            ...
            break
        _save_batch_image(images, labels.tolist(), None,
                          save_dir / f"train_batch{i}.jpg", class_names)
    ...
    for i in range(n_batches):
        ...
        _save_batch_image(images, labels.tolist(), None,
                          save_dir / f"val_batch{i}_labels.jpg", class_names)
        _save_batch_image(images, labels.tolist(), preds,
                          save_dir / f"val_batch{i}_pred.jpg", class_names)
