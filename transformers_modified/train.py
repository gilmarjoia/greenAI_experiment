"""
train.py — Training loop for transformers_modified.

Changes vs transformers_baseline:
  - CrossEntropyLoss now accepts label_smoothing (default 0.1) to reduce
    overconfidence and improve generalisation.
  - gradient clipping (max_norm=1.0) added — standard practice for ViT
    fine-tuning to stabilise training.
  - All other logic (AdamW, cosine LR decay, AMP, CSV logging) is identical
    to transformers_baseline so results remain directly comparable.
"""
