"""
main_round3.py — Orchestrator for the GreenAI Round 3 experiment phase.

Runs the three Round 3 models in sequence:
    1. YOLO26  (yolo26_round3/run.py)
    2. CNN     (cnn_round3/run.py)
    3. ViT     (transformers_round3/run.py)

Each Round 3 run:
  - Manages its own CodeCarbon tracker internally
  - Writes artifacts under  <model>_round3/output/runs/train/
  - Saves an emissions.csv   under  <model>_round3/output/

After all runs, report_round3.py is called to generate a summary table,
followed by generate_final_report.py to produce the consolidated 3-phase report.

Logging:
  - A timestamped log file is written to  logs/experiment_round3_<YYYYMMDD_HHMMSS>.log
  - All console output is mirrored to the log file in real-time

Usage:
    python main_round3.py
...
LOG_FILE = LOGS_DIR / f"experiment_round3_{_RUN_TS}.log"
...
log = logging.getLogger("greenai_round3")

# ─── Round 3 model definitions ────────────────────────────────────────────────
ROUND3_RUNS = [
    {
        "name":   "YOLO26 Round 3",
        "script": ROOT / "yolo26_round3"       / "run.py",
        "cwd":    ROOT / "yolo26_round3",
    },
    {
        "name":   "CNN Round 3",
        "script": ROOT / "cnn_round3"          / "run.py",
        "cwd":    ROOT / "cnn_round3",
    },
    {
        "name":   "ViT (Transformers) Round 3",
        "script": ROOT / "transformers_round3"  / "run.py",
        "cwd":    ROOT / "transformers_round3",
    },
]
...
def run_model(model_run: dict) -> bool:
    """
    Run a single Round 3 script as a subprocess.
    ...
    _banner(f"Running Round 3 model: {name}")
...
def main() -> None:
    _banner("GreenAI Experiment — Sequential Round 3 Runner", width=70)
    log.info("Log file  : %s", LOG_FILE)
    log.info("Run ID    : %s", _RUN_TS)
    log.info("Order     : YOLO26 Round 3 → CNN Round 3 → ViT Round 3")

    experiment_start = time.time()
    statuses: list[tuple[str, bool, float]] = []

    for model_run in ROUND3_RUNS:
        t0      = time.time()
    ...
    # ── Generate Round 3 summary report ──────────────────────────────────────
    _banner("Generating Round 3 Summary Report", width=70)
    run_report(ROOT / "report_round3.py", "report_round3")

    # ── Generate final consolidated report ────────────────────────────────────
    _banner("Generating Final Consolidated Report (3 Phases)", width=70)
    run_report(ROOT / "generate_final_report.py", "generate_final_report")

    # ── Final summary ─────────────────────────────────────────────────────────
    total_elapsed = time.time() - experiment_start
    _banner("Round 3 Experiment Summary", width=70)
    ...
    log.info("Log file                 : %s", LOG_FILE)
    log.info("Round 3 report           : %s", ROOT / "reports" / "summary_report_round3.md")
    log.info("Final consolidated report: %s", ROOT / "reports" / "final_report.md")
    log.info("Final dashboard          : %s", ROOT / "reports" / "final_dashboard.png")

    all_ok = all(ok for _, ok, _ in statuses)
    if not all_ok:
        log.warning("One or more Round 3 runs FAILED. Review the log above.")
        sys.exit(1)
    else:
        log.info("All Round 3 runs completed successfully.")
        sys.exit(0)
