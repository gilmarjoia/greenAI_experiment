"""
val.py — Evaluation script for the CNN baseline.

Follows YOLO26 guidelines:
  - Loads weights from output/runs/train/weights/best.pt
  - Tracks emissions via CodeCarbon: 'load dataset' and 'validate model'
  - Saves artifacts to output/runs/val/
"""
...
def load_config(train_dir: Path) -> dict:
    ...
    if args_path.exists():
        ...
        # Fallback defaults if args.yaml is missing
        return {
            "batch": 16,
            "workers": 8,
            "imgsz": 28,
...
def main():
    ...
    tracker = EmissionsTracker(
        project_name="cnn_baseline_validation",
    ...
    try:
        ...
        # For batch images, we need a dummy train_loader or just pass None if plots.py handles it
        # Looking at plots.py: plot_batch_images(train_loader, val_loader, ...)
        # It uses train_loader to show some training samples. We can pass an empty list or the val_loader again.
        # To be safe and follow the 'val' directory logic, we'll just show val samples.
