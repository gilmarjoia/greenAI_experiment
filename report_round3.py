"""
report_round3.py — Summary report generator for the GreenAI Round 3 experiment.

Reads per-model artifacts produced by run.py:
  • <model>_round3/output/runs/train/results.csv  — training metrics (last epoch)
  • <model>_round3/output/emissions.csv           — CodeCarbon energy/emissions data

Outputs (written to reports/):
  • summary_metrics_round3.csv    — machine-readable table (metrics + emissions)
  • summary_report_round3.md      — human-readable Markdown report

Usage (standalone):
    python report_round3.py
"""
...
ROUND3_MODELS = {
    "YOLO26 Round 3": {
        "results_csv":   ROOT / "yolo26_round3"       / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "yolo26_round3"       / "output" / "emissions.csv",
    },
    "CNN Round 3": {
        "results_csv":   ROOT / "cnn_round3"          / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "cnn_round3"          / "output" / "emissions.csv",
    },
    "ViT (Transformers) Round 3": {
        "results_csv":   ROOT / "transformers_round3"  / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "transformers_round3"  / "output" / "emissions.csv",
    },
}
...
def collect_results() -> list[dict]:
    """Collect metrics and emissions for every Round 3 model."""
    rows = []

    for name, paths in ROUND3_MODELS.items():
...
def write_markdown(rows: list[dict], path: Path) -> None:
    ...
    md = f"""# GreenAI Experiment — Round 3 Summary Report
...
def main() -> None:
    ...
    print("\n" + "=" * 60)
    print("  Generating experiment summary report for Round 3 models")
    print("=" * 60)
    ...
    write_csv(rows,      REPORTS_DIR / "summary_metrics_round3.csv")
    write_markdown(rows, REPORTS_DIR / "summary_report_round3.md")
