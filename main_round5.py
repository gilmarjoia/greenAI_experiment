"""
main_round5.py — Orchestrator for the GreenAI Round 5 experiment phase.

Runs the three Round 5 models in sequence:
    1. YOLO26  (yolo26_round5/run.py)
    2. CNN     (cnn_round5/run.py)
    3. ViT     (transformers_round5/run.py)

Goal of Round 5 — 20 Epoch Baseline:
  Each model uses the baseline hyperparameters (without regularization or augmentation
  improvements) but trains for 20 epochs.

Each Round 5 run:
  - Manages its own CodeCarbon tracker internally
  - Writes artifacts under  <model>_round5/output/runs/train/
  - Saves an emissions.csv   under  <model>_round5/output/

After all runs, report_round5.py is called to generate a summary table.

Logging:
  - A timestamped log file is written to  logs/experiment_round5_<YYYYMMDD_HHMMSS>.log
  - All console output is mirrored to the log file in real-time

Usage:
    python main_round5.py
"""
...
LOG_FILE = LOGS_DIR / f"experiment_round5_{_RUN_TS}.log"
...
log = logging.getLogger("greenai_round5")

# ─── Round 5 model definitions ────────────────────────────────────────────────
ROUND5_RUNS = [
    {
        "name":   "YOLO26 Round 5",
        "script": ROOT / "yolo26_round5"       / "run.py",
        "cwd":    ROOT / "yolo26_round5",
    },
    {
        "name":   "CNN Round 5",
        "script": ROOT / "cnn_round5"          / "run.py",
        "cwd":    ROOT / "cnn_round5",
    },
    {
        "name":   "ViT (Transformers) Round 5",
        "script": ROOT / "transformers_round5"  / "run.py",
        "cwd":    ROOT / "transformers_round5",
    },
]
...
def run_model(model_run: dict) -> bool:
    """
    Run a single Round 5 script as a subprocess.
    ...
    _banner(f"Running Round 5 model: {name}")
...
def main() -> None:
    _banner("GreenAI Experiment — Sequential Round 5 Runner (20 Epoch Baseline)", width=70)
    log.info("Log file  : %s", LOG_FILE)
    log.info("Run ID    : %s", _RUN_TS)
    log.info("Order     : YOLO26 Round 5 → CNN Round 5 → ViT Round 5")
    log.info("Goal      : Baseline parameters + 20 epochs")

    experiment_start = time.time()
    statuses: list[tuple[str, bool, float]] = []

    for model_run in ROUND5_RUNS:
        t0      = time.time()
    ...
    # ── Generate Round 5 summary report ──────────────────────────────────────
    _banner("Generating Round 5 Summary Report", width=70)
    run_report(ROOT / "report_round5.py", "report_round5")

    # ── Final summary ─────────────────────────────────────────────────────────
    total_elapsed = time.time() - experiment_start
    _banner("Round 5 Experiment Summary", width=70)
    ...
    log.info("Log file              : %s", LOG_FILE)
    log.info("Round 5 report        : %s", ROOT / "reports" / "summary_report_round5.md")

    all_ok = all(ok for _, ok, _ in statuses)
    if not all_ok:
        log.warning("One or more Round 5 runs FAILED. Review the log above.")
        sys.exit(1)
    else:
        log.info("All Round 5 runs completed successfully.")
        sys.exit(0)
