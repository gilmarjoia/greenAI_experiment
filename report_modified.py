"""
report_modified.py — Summary report generator for the GreenAI modified experiment.

Reads per-modified model artifacts produced by run.py:
  • <model>_modified/output/runs/train/results.csv  — training metrics (last epoch)
  • <model>_modified/output/emissions.csv           — CodeCarbon energy/emissions data

Outputs (written to reports/):
  • summary_metrics_modified.csv    — machine-readable table (metrics + emissions)
  • summary_report_modified.md      — human-readable Markdown report

Usage (standalone):
    python report_modified.py
"""
...
MODIFIED_MODELS = {
    "YOLO26 Modified": {
        "results_csv":   ROOT / "yolo26_modified"      / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "yolo26_modified"      / "output" / "emissions.csv",
    },
    "CNN Modified": {
        "results_csv":   ROOT / "cnn_modified"         / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "cnn_modified"         / "output" / "emissions.csv",
    },
    "ViT (Transformers) Modified": {
        "results_csv":   ROOT / "transformers_modified" / "output" / "runs" / "train" / "results.csv",
        "emissions_csv": ROOT / "transformers_modified" / "output" / "emissions.csv",
    },
}
...
def collect_results() -> list[dict]:
    """Collect metrics and emissions for every modified model."""
    rows = []

    for name, paths in MODIFIED_MODELS.items():
...
def write_markdown(rows: list[dict], path: Path) -> None:
    ...
    md = f"""# GreenAI Experiment — Modified Models Summary Report
...
def main() -> None:
    ...
    print("\n" + "=" * 60)
    print("  Generating experiment summary report for modified models")
    print("=" * 60)
    ...
    write_csv(rows,      REPORTS_DIR / "summary_metrics_modified.csv")
    write_markdown(rows, REPORTS_DIR / "summary_report_modified.md")
